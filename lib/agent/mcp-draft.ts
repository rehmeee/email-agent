...
import {
  resolveAttachmentsToMcpContent,
} from "@/lib/agent/mcp-attachments";
 ...
 * Resolves Drive attachments via authenticated Drive API → base64 content
 * (never Drive export URLs — MCP URL fetch returns 401 without Google Bearer).
 */
export async function createGmailDraftViaMcp(input: {
  ...
  if (input.draft.attachments?.length) {
    args.attachments = await resolveAttachmentsToMcpContent(
      accessToken,
